...
public class CameraActivity extends ActivityBase implements
    ...
    protected void onResume() {
        ...
        CameraPerformanceTracker.onEvent(TAG,
                ...
                CameraPerformanceTracker.ISBEGIN);
        super.onResume();
		// A : DSYYLM-280, kui.li , 2016-01-20,for brightness control, start {
		setBrightnessWhenResume();
		// A : end }
        if (FeatureSwitcher.isApi2Enable(this)) {
            if (mPermissionManager.requestCameraLaunchPermissions()) {
                mIsAPI2Inited = true;
    ...
    private void updateCameraLocationInfo() {
        ...
        if (!mIsCheckingLocationPermission) {
            ...
            if (isLocationPermissionReady) {
            	// bug-id:DWYSLM-10 modify by anxiang.xiao add warning box for user to turn on the location info 20160118  start
				String location = "";
				location = Settings.System.getString(getContentResolver(),	Settings.System.LOCATION_PROVIDERS_ALLOWED);
				...
				if (!(location.contains("gps") || location.contains("network"))) {
					mCameraAppUi.showAlertDialog(null,	getString(R.string.location_need_enable_warning_msg),
									getString(android.R.string.cancel), null,
									getString(android.R.string.ok), runnable);
				}
				// bug-id:DWYSLM-10 modify by anxiang.xiao add warning box for user to turn on the location info 20160118  end
				mLocationManager.recordLocation(true);
            } else {
                /**
        ...
        public void onModeChanged(int newMode) {
            ...
            if (mCameraActor.getMode() != newMode) {
                ...
                boolean needRestart = (!oldCameraMode.equals(newCameraMode))
                        || Parameters.CAMERA_MODE_MTK_VDO == Integer
    ...
    private PickerManager.PickerListener mPickerListener = new PickerManager.PickerListener() {
        ...
        public boolean onCameraPicked(int cameraId) {
            ...
            mCameraAppUi.setViewState(ViewState.VIEW_STATE_CAMERA_CLOSED);
            mMainHandler.obtainMessage(MSG_SWITCH_CAMERA, cameraId, 0)
                    .sendToTarget();
	...
	private int lightSensorAutoMode ;
	private boolean lightSensorAutoModeChange = false;
    private void setBrightnessWhenResume(){
	  lightSensorAutoMode = getBrightnessAutoMode();
	  Log.e(TAG,"setBrightnessWhenResume(),lightSensorAutoMode="+lightSensorAutoMode);
	  if (lightSensorAutoMode == 1){
	  	 lightSensorAutoModeChange = true;
		 setBrightnessAutoMode(0);
	  }
	  userBrightness = getBrightness();
	  setBrightness(getResources().getInteger(com.android.internal.R.integer.config_screenBrightnessSettingMaximum));
    }	
    private void resetBrightnessWhenPause(){
	  Log.e(TAG,"resetBrightnessWhenPause(),lightSensorAutoMode="+lightSensorAutoMode);
	  if (lightSensorAutoModeChange){
	  	 lightSensorAutoModeChange = false;
		 setBrightnessAutoMode(lightSensorAutoMode);
	  }	
    ...
    private int getBrightness() {
        return Settings.System.getInt(getContentResolver(), Settings.System.SCREEN_BRIGHTNESS,getResources().getInteger(com.android.internal.R.integer.config_screenBrightnessDim));
    }
    /**
     * 
     * @param mode
     *            auto?1:0
     */
    private void setBrightnessAutoMode(int mode) {
     ...
     */
    private int getBrightnessAutoMode() {
